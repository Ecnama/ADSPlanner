...
test_that("assign_depart_hard only affects selected students", {
    ...
    expect_equal(result$Aff_depart_1, c(parse_file_output$V1[1], parse_file_output$V1[2], NA_character_))
})
...
test_that("assign_depart_erase only affects selected students", {
    result <- assign_depart_hard(parse_file_output, c(1, 2, 3), 1)
    result <- assign_depart_erase(result, c(1, 2))
    expect_equal(result$Aff_depart_1, c(NA_character_, NA_character_, parse_file_output$V1[3]))
})
